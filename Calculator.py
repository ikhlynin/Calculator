...
class Calculator(App):
    ...
    def add_operator(self, instance):
        ...
        if self.formula[sz] == "" or self.formula[sz] == "+" or self.formula[sz] == "-" or self.formula[sz] == "*" or self.formula[sz] == "/":
            self.formula += ""
        else:
            self.formula += str(instance.text)
        self.update_lable()
    ...
    def build(self):
        ...
        self.lbl = Label(text = "0",color = (0,0,0,1), font_size = 45, halign="right", size_hint = (1.5, .4), text_size = (400 - 50, 500 * .4 - 50))
        ...
        gl.add_widget( Button(color=(0, 0, 0, 1), background_color=(.9, .9, .9, .3),text = "=", on_press = self.culc_result) )
        gl.add_widget( Button(color=(0, 0, 0, 1), background_color=(.9, .9, .9, .3),text = "-", on_press = self.add_operator) )
        ...
        gl1.add_widget( Button(color=(0, 0, 0, 1), background_color=(.9, .9, .9, .3),text = "CE", on_press = self.clear_one) )
